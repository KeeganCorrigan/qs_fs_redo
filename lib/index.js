...
const renderTotalCalories = (totalCalories) => {
  $('#total-calories').html(`<p> ${totalCalories}</p>`)
}
...
const appendMealFoods = (food, id) => {
  console.log("in appendMealFoods", food.id, id)
  $('#meals-table-index').append(`
    ...
    <button type="button" class="remove-from-meal" data-food-id="${food.id}" data-meal-id="${id}">Remove</button>
    `)
};
